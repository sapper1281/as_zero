...
@ManagedBean
@ViewScoped
public class SaveUserPageBean1 implements Serializable {
    ...
    private void updateInfo() {
        ...
        if (item == null || item.getId() != id && id >= 0 || (id==0 && (item.getDepartment()==null || item.getDepartment().getId()!=dep))) {
    ...
    public String saveItem() {
        try {
            ...
            try {
                ...
                Logger.getLogger(SaveUserPageBean1.class.getName()).log(Level.SEVERE, null, ex1);
            }
        ...
        //dep.setAddress(Constants.DEPARTMENT_URL);
        setTab(1);
        return "";
    ...
    public String deleteItem() {
        try {
            deleteUser(item);
            dep.setMessage("удален");
            org.apache.log4j.Logger.getLogger(DepartmentPageBeanNew.class.getName()).log(org.apache.log4j.Level.ERROR, "Удален П: "+"("+item.getId()+")"
                +item.getFIO()+" адм. "+session.getUser().getUserID());
        } catch (ExistingUserExeption ex) {
            session.setError(ex.getMessage());
            return "errorpage.xhtml";
        } catch (Exception ex) {
     ...
     *
     * @throws ExistingUserExeption если в БД уже есть пользователь с логином,
     * на который хотим поменять
     */
