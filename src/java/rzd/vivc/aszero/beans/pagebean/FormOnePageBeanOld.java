...
@ManagedBean
@ViewScoped
public class FormOnePageBeanOld extends BasePage implements Serializable {

    public FormOnePageBeanOld() {
        super();
    }
...
//        System.out.println(""+rep.getPlan_col());
   ...
   public void addMessage() {
    ...
    private Date getDT_begin() {
        Date dt_b = new Date();
        dt_b.setTime(rep.getTime_begin().getTime() + rep.getDt_begin().getTime() - session.getUser().getNum() * 60 * 1000 + 10800000);
        return dt_b;
    }
    ...
    private Date getDT_end() {
        Date dt_f = new Date();
        dt_f.setTime(rep.getTime_finish().getTime() + rep.getDt_begin().getTime() - session.getUser().getNum() * 60 * 1000 + 10800000);
        return dt_f;
    }
    ...
    public void Init() {
        ...
        if (FacesContext.getCurrentInstance().getExternalContext().getRequestParameterMap().containsKey("id")) {
            ...
            if (FacesContext.getCurrentInstance().getExternalContext().getRequestParameterMap().containsKey("form_id")) {
                ...
                /* if (dt_max == null) {
                 Date now = new Date();
                 rep.setDt_begin(getDateFRIDAY(now));
                 } else {
                 Calendar c = Calendar.getInstance();
                 c.setTime(dt_max);
                 c.set(Calendar.DATE, 1);
                ...
                try {
                    ...
                    Logger.getLogger(FormOnePageBeanOld.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
        }
//список всех неудаленных ТЭР звлекаем из БД
//  try {
