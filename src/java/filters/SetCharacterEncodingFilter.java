 ...
 * following initialization parameters: </p> <ul> <li><strong>encoding</strong>
 * - The character encoding to be configured for this request, either
 * conditionally or unconditionally based on the
 * <code>ignore</code> initialization parameter. This parameter is required, so
 * there is no default.</li> <li><strong>ignore</strong> - If set to "true", any
 * character encoding specified by the client is ignored, and the value returned
 * by the
 ...
 * "true".</li> </ul>
 *
 * <p> Although this filter can be used unchanged, it is also easy to subclass
 * it and make the
 * <code>selectEncoding( )</code> method more intelligent about what encoding to
...
public class SetCharacterEncodingFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        if (ignore || (request.getCharacterEncoding() == null)) {
            String encodingg = selectEncoding(request);
            if (encodingg != null) {
                request.setCharacterEncoding(encodingg);
            }
        }
